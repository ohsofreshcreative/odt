...
function custom_posts_pagination_args($args) {
    ...
    $args['screen_reader_text'] = __('Nawigacja po wpisach');
    return $args;
}
add_filter('navigation_markup_template', 'custom_pagination_navigation_template', 10, 2);

function custom_pagination_navigation_template($template, $class) {
    // Add custom classes to the navigation container
    return '
...
add_filter('paginate_links_output', 'custom_paginate_links_output', 10, 1);

function custom_paginate_links_output($output) {
    // Add custom classes to each link/span
