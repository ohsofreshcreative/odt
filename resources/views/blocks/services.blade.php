...
<section x-data @if(!empty($section_id)) id="{{ $section_id }}" @endif x-ref="servicesSection" class="b-services -smt {{ $sectionClass }} {{ $section_class }}">
    <div class="c-main">
        @if ($title)
        ...
        <div x-data="{ activeTab: {{ $service_tabs[0]['term']->term_id }} }" class="mt-12">
            ...
            <div
                ...
                <button
                    @click="activeTab = {{ $tab['term']->term_id }}; $refs.servicesSection.scrollIntoView({ behavior: 'smooth' })"
                    :class="{
